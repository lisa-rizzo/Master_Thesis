...
def load_label_override_step1(
    filter_excel_path: str,
    anomaly_excel_path: str,
    has_T1_col: str = "has_T1",
    is_consec_col: str = "is_consecutive",
) -> dict[str, dict[int, int]]:
    """
    Step 1 of LabelOverride: returns {pid: {disk_vert: override_vert}} for PIDs
    that pass has_T1==1 & is_consecutive==1 (any split) and have a non-null
    LabelOverride in the anomaly Excel.

    The mapping is positional: sorted(disk_verts)[i] -> LabelOverride[i].
    Join key: anomaly.fid == filter.pid  AND  anomaly.dataset == filter.dsname.
    """
    filter_df = pd.read_excel(filter_excel_path)
    filter_df = filter_df.loc[
        (filter_df[has_T1_col] == 1) & (filter_df[is_consec_col] == 1)
    ]

    anomaly_df = pd.read_excel(anomaly_excel_path)
    anomaly_df = anomaly_df[anomaly_df["LabelOverride"].notna()]

    result: dict[str, dict[int, int]] = {}
    for _, row in anomaly_df.iterrows():
        fid    = str(row["fid"]).strip()
        dsname = str(row["dataset"]).strip()
        override_seq = ast.literal_eval(row["LabelOverride"]) \
            if isinstance(row["LabelOverride"], str) else list(row["LabelOverride"])
        override_seq = [int(v) for v in override_seq]

        match = filter_df.loc[
            (filter_df["pid"] == fid) & (filter_df["dsname"] == dsname)
        ]
        if match.empty:
            continue  # not in filtered data (test-only or excluded)

        verts_val = match.iloc[0]["vert_label"]
        disk_verts = ast.literal_eval(verts_val) \
            if isinstance(verts_val, str) else list(verts_val)
        disk_verts_sorted = sorted(int(v) for v in disk_verts if pd.notna(v))

        if len(disk_verts_sorted) != len(override_seq):
            print(f"[LabelOverride] length mismatch for {fid} ({dsname}): "
                  f"{len(disk_verts_sorted)} disk verts vs {len(override_seq)} override — skipping.")
            continue

        result[fid] = {d: c for d, c in zip(disk_verts_sorted, override_seq)}
